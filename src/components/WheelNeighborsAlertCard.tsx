...
export const WheelNeighborsAlertCard: React.FC<WheelNeighborsAlertCardProps> = ({ spins }) => {
  ...
  const targetNum = lastSpin ? lastSpin.number : 0;
    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-xl">
      <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2.5 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-lg border border-slate-800">
          <span className="text-[10px] font-bold text-slate-400 px-1.5">Vizinhos:</span>
      ...
      <div className="bg-slate-950/80 rounded-xl p-3 border border-slate-800/80 mb-3">
        ...
        <div className="flex items-center justify-center gap-1.5 flex-wrap py-1">
          {currentNeighbors.map((num) => {
            const isCenter = num === targetNum;
            return (
              <div
                key={num}
                className={`flex flex-col items-center justify-center rounded-lg px-2.5 py-1.5 transition-all ${
                  isCenter
                    ? 'bg-amber-500/20 border-2 border-amber-400 text-amber-300 font-black shadow-lg scale-105'
                    : 'bg-slate-900 border border-slate-700/80 text-slate-200 font-bold'
                }`}
              >
                <span className={`text-sm ${isCenter ? 'text-amber-300 font-black' : 'text-slate-100'}`}>
                  {num}
                </span>
      ...
      {/* Alert Banner */}
      {alertData?.hasAlert ? (
        <div className="bg-gradient-to-r from-amber-950/40 via-slate-900 to-rose-950/30 p-3 rounded-xl border border-amber-500/40 flex items-start gap-2.5 shadow-md">
          ...
          <div className="space-y-1">
            <span className="text-xs font-extrabold text-amber-300 flex items-center gap-1">
              <Sparkles className="w-3.5 h-3.5" /> ALERTA DE REPETIÇÃO EM VIZINHOS DETECTADO!
            </span>
            <p className="text-xs text-slate-200 leading-relaxed font-medium">
              {alertData.alertMessage}
            </p>
            <div className="text-[11px] font-bold text-emerald-400 bg-slate-950/80 px-2.5 py-1 rounded-lg border border-slate-800 mt-1 inline-block">
              💰 Aposta Sugerida: R$ {chipValue.toFixed(2)} por casa em [{currentNeighbors.join(', ')}] (Total R$ {totalBet.toFixed(2)})
            </div>
          </div>
        </div>
      ) : (
        <div className="bg-slate-950/40 p-2.5 rounded-xl border border-slate-800/60 flex items-center justify-between text-xs text-slate-400 gap-2 flex-wrap">
          <span className="flex items-center gap-1.5 font-medium">
            <AlertCircle className="w-3.5 h-3.5 text-slate-400" />
            Monitore a frequência no setor do #{targetNum}. Se os vizinhos repetirem, um alerta surgirá aqui!
          </span>
          <span className="text-[10px] font-black text-amber-400 bg-slate-900 px-2 py-0.5 rounded border border-slate-800">
